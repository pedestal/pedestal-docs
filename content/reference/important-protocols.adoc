...
itself and a link:request-map[request map]. It must return a
link:route-map[route map] for the route that was matched.
...
| :map-tree      | link:../api/pedestal.route/io.pedestal.http.route.map-tree.thml#var-map-tree[`map-tree/router`]
...
link:context-map[context map] and enquque
