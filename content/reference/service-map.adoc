...
If the ::http/allowed-interceptors key is non-`nil`, this interceptor
...
When a value for `::http/enable-csrf` is present, the CSRF interceptor is added to the stack. This implies sessions are enabled. The value must be a map with the following keys:
