...
up correctly. Pedestal provides the test helper `response-for` utility
function to facilitate testing of services without having to spin up
an http server.
...
includes the `io.pedestal.http.body-params/body-params` interceptor to
support request payload parsing. Therefore, you will need to set the
`Content-Type` header of the test request to the appropriate value
based on the payload format of the request body.
