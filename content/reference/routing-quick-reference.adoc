...
`:route-name` must be unique, often it is a qualified keyword to ensure uniqueness.
...
----
<1> Note the use of a backtick; this ensures that the `list-orders` symbol is expanded to a fully qualified symbol,
not the value of `list-orders`.  This fully qualified symbol will be used to derive the route name as a fully qualified
...
to a function, the route name will be derived from
...
The terse syntax is designed to remove a fair amount of repetition in the table symtax;
...
Pedestal includes several different Routers; this reflects not only the evolution of the Pedestal library,
...
When your application starts a Pedestal service with link:../api/io.pedestal.http.html#var-create-servlet[`create-servlet`] or link:../api/io.pedestal.http.html#var-create-server[`create-server`], Pedestal creates a router, using the following keys from the service map:
...
When the router does not match any route, then the context key `:route` is set to nil.
