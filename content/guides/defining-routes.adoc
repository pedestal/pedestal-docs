...
--
Path parameter decoding is the responsibility of the
link:../api/io.pedestal.http.route.html#var-path-params-decoder[path-params-decoder] interceptor (since
version `0.5.4`). However, the path decoder interceptor is placed too early in the default interceptor list,
so it executes before the router has executed, and the router is what stores the path params into the request.
If your applications requires path parameters to be decoded, you must explicitly
add the `path-params-decoder` to your interceptor list, at the end (after the router).

Refer to issue
link:https://github.com/pedestal/pedestal/issues/588[588] for more info.
--
...
link:../api/io.pedestal.http.body-params.html#var-body-params[body-params] is a
...
A route name must be a keyword.   Most commonly, it is a namespaced qualified keyword.
...
function
link:../api/io.pedestal.http.route.html#var-url-for[`url-for`]
calls the dynamically bound function. This way, you can call
link:../api/io.pedestal.http.route.html#var-url-for[`url-for`] from any thread that is currently
...
link:../api/io.pedestal.http.route.html#var-form-action-for-routes[`form-action-for-routes`]
