...
from the link:../api/io.pedestal.http.html[pedestal.service]
module. Interceptor invocation and handling comes from
link:../api/io.pedestal.interceptor.html[pedestal.interceptor]
which doesn't depend at pedestal.service.
...
link:../api/io.pedestal.interceptor.chain.html#var-execute[io.pedestal.interceptor.chain/execute] function is invoked.
...
Let's rewrite `number-format-handler` using to `error-dispatch` to create the interceptor for us.
...
- A fully qualified symbol is passed to `resolve`; the result is then turned into an Interceptor
...
The link:../api/io.pedestal.interceptor.html#var-interceptor[interceptor] function is used to
