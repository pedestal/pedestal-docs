...
== Before We Begin
Let's consider our component's responsibilities since they will
drive it's design.

Starting a server is an easy one. Without that we don't have a service.

Provider initialization is also necessary but may be less apparent
since Pedestal's bootstrapping function,
`io.pedestal.http/create-server`, tends to buffer client code from
that.

A Pedestal service function (`:io.pedestal.http/service-fn`) will be
exposed upon component initialization. This makes the component
amenable to testing through Pedestal's testing facilities.

That's really all we need to get started. Notice how Route or
interceptor management was left out. Although it may be tempting to
include them as component responsibilities, the management of routes
or interceptors is more of an application-specific concern. This may
be familiar to you if you studied the way the Pedestal _lein template_
lays out a Pedestal service - configuration is kept separate from
service plumbing. Since Pedestal service configuration is captured via a
link::service-map[Service Map], we'll make our Pedestal component
