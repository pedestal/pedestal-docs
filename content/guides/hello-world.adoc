...
tells Clojure what namespace to put the code into. (If you're a Java
...
1. Listen on a socket for HTTP requests.
...
will translate that into a full HTTP response complete with a content
...
----
<1> Tell `clj` we need three Pedestal libraries and a logging library.
...
namespace `hello` is an object. It happens to be a function object. Why
...
This one route says that incoming HTTP GET requests with the URI path 
'/greet' should call the function `respond-hello`, and that
...
so see how it would be routed;  return an expanded routing entry if matched.
...
that a GET to "/greet" will be a match, but a PUT or POST will not. When
...
Importantly, one part of the data returned is the `:interceptor` list; these are interceptors that, in normal operation,
Pedestal would queue up to do the work of the matched route.
