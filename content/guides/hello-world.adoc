...
adding the library for your chosen service. We're using Jetty for this
...
Yep, it didn't return. Jetty is running and listening for connections, though. Flip to a different window and try curl or try http://127.0.0.1:8890/greet.
...
It's alive! Treat yourself to a hot beverage and a high five.
...
The link:hello-world-query[next part] in this trail adds the ability
to receive a query parameter, apply logic to it, and return an HTML or
JSON page.
