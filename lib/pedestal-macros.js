...
module.exports = function (registry) {
    ...
    // Asciidoctor folds `->` into a single char; this inserts \ escape characters to defeat that.
    const to_label = (s) => s.replaceAll(/\-[<>]/g, (m) => "\\" + m);
    ...
    registry.inlineMacro("clj", function () {
        ...
        self.process(function (parent, target, attributes) {
            ...
            const label = to_label(name);
            const namespace = attributes.ns || "clojure.core";

            var text = `link:https://clojure.github.io/clojure/${namespace}-api.html#${namespace}/${target}[${quote}${label}${quote}]`;
